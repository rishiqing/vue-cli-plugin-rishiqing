...
const payFunctions = {
  ...
  openPayPage: () => {
    ...
    console.warn('必须接入到主工程之后才能打开版本升级页面')
  },
  // 打开付费页面
  openPayDetail: () => {
    // eslint-disable-next-line no-console
    console.warn('必须接入到主工程之后才能打开付费页面')
  },
}
// 打开通讯录邀请的方法
let openInviteMemberFn = () => {
  // eslint-disable-next-line no-console
  console.warn('必须接入到主工程之后才能打开通讯录邀请')
}
...
function convertUserTreeToUnfoldDeptList(deptList, unfoldList, map) {
  function nextDept(dept) {
    ...
    map[dept.id] = dept
    // 如果是未分配部门，则放到 data.UNDEPT 上
    if (dept.type === 'unassigned') {
      data.UNDEPT = dept
    }
    if (dept.childList && dept.childList.length) {
      dept.childList.forEach(nextDept)
...
export async function initData() {
  ...
  if (RISHIQING_SINGLE_SPA) {
    ...
    data.FRESH_TIME = basicData.FRESH_TIME

    // 从shareData获取消息客户端
    if (ShareData.getMessageClient) {
      messageClient = ShareData.getMessageClient()
    }
    // 从shareData获取systemConfig
    if (ShareData.getSystemConfig) {
      systemConfig = ShareData.getSystemConfig()
    }
    // 从shareData获取打开版本升级页面的方法
    if (ShareData.openPayPage) {
      payFunctions.openPayPage = ShareData.openPayPage
    }
    // 从shareData获取打开付费详情页面的方法
    if (ShareData.openPayDetail) {
      payFunctions.openPayDetail = ShareData.openPayDetail
    }
    // 从shareData获取打开通讯录邀请的方法
    if (ShareData.openInviteMember) {
      openInviteMemberFn = ShareData.openInviteMember
    }
  }
  ...
  if (!RISHIQING_SINGLE_SPA && __DEV__) {
    ...
    try {
      ...
      data.FRESH_TIME = `${data.USER_INFO.id}${new Date().getTime()}`
...
export function getDeptListByIds(ids) {
  return ids.map(id => data.DEPT_LIST_MAP[id]).filter(dept => dept)
}
...
export function getUserListByIds(ids) {
  return ids.map(id => data.USER_LIST_MAP[id]).filter(user => user)
}

/**
 * 获取未分配部门
 */
export function getUnDept() {
  return data.UNDEPT
}
...
export function openPayPage() {
  return payFunctions.openPayPage()
}

/**
 * 打开付费详情页面
 * @param {string} buyType 想要购买的版本类型(ZY: 专业版, QY: 企业版, QJ: 旗舰版), 不区分大小写.如果不传buyType，则默认为续费操作
 */
export function openPayDetail(buyType) {
  return payFunctions.openPayDetail(buyType)
}
/**
 * 打开通讯录邀请
 * param {string} type为打开的邀请类型，(add: 添加成员弹窗，包含直接和批量邀请; invite: 邀请成员弹窗，包含链接邀请,团队码邀请以及二维码邀请),不传type则默认为add，即默认打开添加成员弹窗
 */
export function openInviteMember(type) {
  return openInviteMemberFn(type)
}
